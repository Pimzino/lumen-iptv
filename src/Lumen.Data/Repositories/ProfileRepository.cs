...
public sealed class ProfileRepository : IProfileRepository
{
    ...
    public async Task<Profile?> GetAsync(long id, CancellationToken cancellationToken)
    {
        var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using (connection.ConfigureAwait(false))
        {
    ...
    public async Task<long> InsertAsync(Profile profile, CancellationToken cancellationToken)
    {
    ...
    public async Task UpdateAsync(Profile profile, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(profile);
        var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using (connection.ConfigureAwait(false))
    ...
    public async Task TouchLastUsedAsync(long id, long unixSeconds, CancellationToken cancellationToken)
    {
        var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using (connection.ConfigureAwait(false))
        {
